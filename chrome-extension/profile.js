...
function setCursorAndScroll(editor, newContent, cursorPosition) {
  ...
  const lineHeight = parseInt(window.getComputedStyle(editor).lineHeight) || DEFAULT_LINE_HEIGHT;
  const scrollPosition = Math.max(0, (linesBefore - SCROLL_OFFSET_LINES) * lineHeight);
...
function insertTextAtPosition(content, insertPos, text) {
  const newContent = content.slice(0, insertPos) + text + '\n' + content.slice(insertPos);
...
function findEntryEndPosition(content, entryStartPos) {
  ...
  if (nextEntryMatch && (!nextSectionMatch || nextEntryMatch.index < nextSectionMatch.index)) {
...
function findSmartInsertPosition(content, startPos, endPos, fieldName, fieldOrder) {
...
function updateValidationUI(validation) {
  ...
  validation.errors.forEach((error, index) => {
    ...
    if (fix && fix.type === 'replace_enum_value_multi') {
      ...
      const messageWithoutValues = error.message.replace(/\. Allowed values:.*$/, '');
...
function applyEnumFix(fix, selectedValue) {
  ...
  const enumRegex = new RegExp(`(##\\s+${fix.entry}[\\s\\S]*?${fix.field}:\\s*)${fix.currentValue}`, 'm');
  ...
  if (enumMatch) {
    ...
    const newContent = content.slice(0, replaceStart) + selectedValue + content.slice(replaceEnd);
  ...
  switch (fix.type) {
    ...
    case 'insert_top_level_field':
      ...
      if (profileMatch) {
        ...
        const insertPos = findSmartInsertPosition(content, topLevelStart, topLevelEnd, fix.field, FIELD_ORDER.TOP_LEVEL);
    ...
    case 'insert_field_in_entry':
      ...
      if (entryMatch) {
        ...
        const insertPos = findSmartInsertPosition(content, entryStart, entryEnd, fix.field, fieldOrder);
...
function formatProfile() {
  ...
  // Match: KEY: value\n\nKEY: value -> KEY: value\nKEY: value
  newContent = newContent.replace(/^(\s*[A-Z_]+:.*)\n\n(\s*[A-Z_]+:)/gm, '$1\n$2');
  ...
  // Match: ## ENTRY_ID\n\nKEY: value -> ## ENTRY_ID\nKEY: value
  newContent = newContent.replace(/^(##\s+[A-Z0-9_]+)\s*\n\n(\s*[A-Z_]+:)/gm, '$1\n$2');
...
`;
  ...
  if (eduSectionMatch) {
    ...
    const newContent = content.slice(0, insertPos) + sectionTemplate + '\n' + content.slice(insertPos);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('fixFormattingBtn').addEventListener('click', formatProfile);
  ...
  document.getElementById('addEducationBtn').addEventListener('click', insertEducationTemplate);
  document.getElementById('addExperienceBtn').addEventListener('click', insertExperienceTemplate);
  
  // Fix buttons (event delegation)
  document.getElementById('validationContent').addEventListener('click', function(e) {
